...
- Run `pagerunner status` to verify setup; `pagerunner daemon &` for multiple Claude windows

## Anonymization (PII protection)

Pass `anonymize: true` to `open_session` to prevent PII from reaching Claude:

```json
{ "profile": "personal", "anonymize": true }
```

- All `get_content` and `evaluate` results have PII stripped before Claude sees them
